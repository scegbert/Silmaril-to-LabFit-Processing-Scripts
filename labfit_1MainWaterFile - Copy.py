...
bin_name = 'B30' # name of working bin (for these calculations)
...
which_ASC_files = ['2207_500_K_8_T_7.asc', '1403_300_K_2_T_3.asc'] # bad ones from earlier test , '1606_300_K_4_T_6.asc'
...
for [prop_which, prop_which2, prop_which3, d_save_name, continuing_features, ratio_min_float] in prop_whiches: 
    ...
    while len(features_reject) > 0 or feature_error is not None or iter_prop <= 2: # until an iteration doesn't reject any features (or feature_error repeats itself)
        ...
        if iter_prop == 1: iter_labfit = 1; unc_multiplier = 1.1
...
if feature_error is None: lab.save_file(d_labfit_main, bin_name, 'added a few new features during last check', d_folder_input=d_labfit_kernal)
