...
bin_breaks = [6500.0, 6562.8, 6579.7, 6599.5, 6620.6, 6639.4, 6660.2, 6680.1, 6699.6, 6717.9,
...
d_labfit = r'C:\Users\scott\Documents\1-WorkStuff\Labfit'
...
use_saved = 'rei_saved' # build on the og rei file
use_inp = 'inp_new'
...
# %% see kiddie pool script to get a labfit inp file started
...
import time
minutes = 30 # delay in hours

for i in range(minutes): 
    print(i)
    time.sleep(60) # delay one hour
...
for [prop_which, prop_which2, prop_which3, d_save_name] in prop_whiches: # run through a bunch of senarios
    ...
    if d_save_name == 'only nu': # use all rejected features
        features_test = a_features_reject.copy()
        features_doublets = a_features_doublets_reject.copy()
            
    elif d_save_name == 'nu cleanup after only sw after only nu' or (d_save_name == 'after n delta self' and len(prop_whiches) > 1): # use all accepted features
    ...
    while len(features_reject) > 0 or feature_error is not None: # until an iteration doesn't reject any features (or feature_error repeats itself)
        ...
        lab.float_lines(d_labfit, bin_name, features, props[prop_which], use_saved, features_constrain) # float lines, most recent saved REI in -> INP out
        if prop_which2 is not False: lab.float_lines(d_labfit, bin_name, features, props[prop_which2], use_inp, features_constrain) # INP -> INP, testing two at once (typically nu or n_self)
        if prop_which3 is not False: lab.float_lines(d_labfit, bin_name, features, props[prop_which3], use_inp, features_constrain, nudge_sd) # INP -> INP, testing two at once (typically sd_self)
        ...
        if feature_error is None: # if we made if through all iterations without a feature causing an error...
            ...
            [df_compare, df_props] = lab.compare_dfs(d_labfit, d_old, bins, bin_name, props[prop_which], props_which, props[prop_which2], plots = False) # read results into python
        ...
        if feature_error is not None: # if labfit wouldn't run, reject that feature
        ...
        for doublet in features_constrain[:]: # check on the doublets (maybe we can float them as a pair), use a copy of list since we're deleting items from it
        ...
        features = list(set(features) - set(features_reject)) # new list of features to float (don't float any that didn't work out)
        
        for featuresi in features_remove_manually: 
            features_reject = [x for x in features_reject if x != featuresi] # remove all instances of problem feature
            
            try: features_reject.remove(featuresi)
            except: pass
        
        if features_reject_old == features_reject and features_reject != []: 
            features_remove_manually.extend(features_reject) # we're stuck in a loop, either stop the loop (throw error) or ignore those features
            
        features_reject_old = features_reject.copy()
        print(features_reject)
        if features == []: iters = 0 # if you're not floating anything, don't bother looping through things as intensely
    ...
    lab.save_file(d_labfit, bin_name, d_save_name) # save file, this is what you will be reloading for the next round of the for loop  
...
if d_save_name == 'nu cleanup after only sw after only nu': prop_which2 = 'sw'
if d_save_name == 'after n delta self': prop_which2 = 'delta_self'

[df_compare, df_props] = lab.compare_dfs(d_labfit, d_old, bins, bin_name, props[prop_which], props_which, props[prop_which2], props[prop_which3]) # <-------------------
...
lab.plot_spectra(T,wvn,trans,res,res_og, df_calcs, offset, props[prop_which], props[prop_which2]) # <-------------------
plt.title(bin_name)
...
lab.save_file(d_labfit, bin_name, 'removed SD0')
