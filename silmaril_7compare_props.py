...
d_meas = r'C:\Users\scott\Downloads'
...
for i, output in enumerate(output_all): 
    ...
    plt.plot(T_all, 100*(T_all - output[:,4]) / T_all, 'x', label=output_names[i])
...
for i, output in enumerate(output_all): 
    ...
    plt.plot(P_all, 100*(P_all - output[:,2]) / P_all, 'x', label=output_names[i])
...
for i, output in enumerate(output_all): 
    ...
    plt.plot(T_all, output[:,6], 'x', label=output_names[i])
